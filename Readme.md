...
1. Data Model Class를 만들어서 기존 dict로 여러 번 복붙했던 코드를 해결하자
   
2. 멀티 프로세싱 풀 같은걸 이용해서 본격적으로 병렬 수집에 대한 공부 시작하기

3. database 쪽 코드 개선하거나 ORM 써보기..?
   - ORM의 필요성을 아직은 느끼지 못해서.. 못 쓰고 있지만 편하다고 하길래.. 써보고 싶어진다.

4. 출시 미정 게임의 게임 출시 날짜가 업데이트가 되었을 때 어떻게 반영할 것인가?
...
|Webdriver|✅|드라이버 오류 발생 시 재시작하기 위한 싱글톤 설계
...
### 해결 방안
- 플랫폼마다 csv 파일을 기준으로 게임이 존재하는지 확인
...
### Webdriver
Selenium에서 자주 사용되는 기능 정리

- 싱글톤 패턴으로 제작해 오류가 발생해 재시작하더라도 원활한 데이터 수집이 가능하도록 제작

- 기존 함수 방식은 생성되었던 웹 드라이버 메모리 주소를 제대로 찾지 못해 웹 드라이버를 찾을 수 없는 현상이 발생했었음
